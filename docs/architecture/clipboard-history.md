...
2. 发现新变更后，在同一同步采集过程中读取类型、来源和内容。优先读取 `org.nspasteboard.source` 的 bundle ID，并用该 ID 作为来源及排除名单键；显式空来源保持未知，不回退到前台应用。只有标记缺失时才回退到当前 `NSWorkspace.shared.frontmostApplication`，1Password 自定义类型继续作为回退来源覆写依据。前台应用是推测，不能证明后台写入者身份。
