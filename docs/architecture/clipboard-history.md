...
自然语言找回是缺省关闭的实验功能，必须先由用户在通用设置中开启，再在搜索框显式触发。关闭时不调用模型、不建立索引，搜索行为保持为关键词搜索。解释器先把描述转换为关键词、来源、类型、日期区间、收藏、备注及 Handoff 条件；用户原句未明确提到的来源、类型和其他布尔条件会被确定性校验丢弃，防止模型猜测额外筛选条件。

语义索引使用独立的 `clip_semantics` 表保存中英文摘要、概念标签和两套句向量。开关开启后分批回填旧历史，新记录写入后异步增量索引；设置页通过独立状态对象展示系统模型可用性、已处理数量和总量。关闭开关会中止后续模型处理，保留的索引不参与搜索。重建入口只清空 `clip_semantics` 后重新生成，不改动 `clips`。正文、标题和备注只取受限前缀交给设备端模型。删除、去重替换和保留策略通过数据库触发器同步清理语义记录。自然语言搜索依次合并 FTS 字面命中、模型概念扩展命中、标签命中和中英文句向量相似候选，最多取 36 条受限正文交给设备端模型重排，再应用日期、来源、类型等确定性条件。精确字面命中不会因重排漏选而消失。

全部模型处理都发生在本机，不调用外部服务。模型不可用、未就绪、索引或重排失败时降级到已有关键词与结构化筛选。诊断只记录状态、数量和耗时，不记录搜索描述、剪贴板正文、摘要、标签或模型输入输出。
