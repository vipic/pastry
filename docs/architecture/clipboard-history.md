...
2. 发现新变更后，在同一同步采集过程中读取类型、来源和内容。优先读取 `org.nspasteboard.source` 的 bundle ID，并用该 ID 作为来源及排除名单键；显式空来源保持未知，不回退到前台应用。只有标记缺失时才回退到当前 `NSWorkspace.shared.frontmostApplication`，1Password 自定义类型继续作为回退来源覆写依据。无来源标记时，排除检查在读取正文前使用一次前台应用，记录归属则在读完载荷后再次获取前台应用，以匹配 Maccy 的取值时机。该时间点可能捕获短暂激活的截图工具，但前台应用仍只是推测，不能证明后台写入者身份。
3. 在读取内容之前过滤 `org.nspasteboard.ConcealedType`、`org.nspasteboard.TransientType`、`org.nspasteboard.AutoGeneratedType` 和命中排除名单的 bundle ID。Handoff 类型会标记远程来源并清空普通应用名。过滤不会修改系统剪贴板，不干扰输入法随后执行粘贴。
4. 内容按用户意图优先级解析：文件 URL、网页 URL、图片、HTML、RTF、纯文本。采集文件 URL 时同时保存文件书签；原路径失效后，预览、打开、在 Finder 中显示、拖拽及写回剪贴板会通过书签继续定位同一卷内被移动的文件。所有格式完成内存读取后再次校验 `changeCount`，变化则丢弃本次结果，下一轮处理新代内容；只有前后计数一致的数据才交给后台图片缓存、富文本解析和历史发布。图片的磁盘写入和富文本解析在后台任务完成，最终回到主线程发布。
...
2026-09-23 核对：来源标记是写入应用自愿提供的约定，并非系统认证。没有来源标记的后台截图、语音输入和自动化写入仍可能被归到前台应用；仅将此类后台应用加入排除名单不能保证拦截。当前没有持久化系统写入方身份，也不声称已解决 CleanShot X 或 Hex 的无标记来源识别。

在 macOS 27 / Apple silicon 上验证了未公开的 `CFPasteboardCopySourceSigningIdentifier` 和 AppKit `_sourceSigningIdentifier`：符号与方法存在，但独立剪贴板的同进程、跨进程写入实验均未取得来源；用户人工复制的 CleanShot X 截图在仅查询身份和变化编号的诊断中也返回空身份，底层状态为 `-21`，前后变化编号一致。继续核对本机系统实现，`com.apple.private.CFPasteboard.get-paste-source` 私有权限检查失败分支返回 `-21`；这不是 Accessibility 或 Input Monitoring 授权。该接口没有公开契约，macOS 26 未实测，不能把本机逆向观察外推为所有系统版本保证。实验没有读取真实剪贴板正文或图片。生产代码不调用这些未公开接口，不因符号存在就启用，也未增加私有 entitlement 或系统权限。

通用标记过滤只能覆盖主动携带标记的软件。Hex 公开源码中的普通文本中转路径没有这些标记，因此本次过滤补齐不能视为 Hex 兼容性验收通过。需要继续验证可用的系统身份能力，或由写入端遵循通用标记约定；不以延迟丢弃、运行中应用名单或图片文件名替代可靠来源。
...
- `ClipboardMonitorTests`：格式优先级、敏感/临时/自动生成类型、排除应用、显式空来源、读取前及读取中代际变化、Handoff 和独立 pasteboard 读取。
