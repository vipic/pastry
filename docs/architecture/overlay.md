...
- `SmartActionPanelManager` / `SmartActionPanelView`：从单条文本或图片卡片打开独立智能操作窗口，协调 Vision 文字识别、动作选择、设备端草稿生成、编辑确认及日历或邮件执行。
...
苹果智能总开关同时控制智能找回和卡片智能操作。关闭时，文本、RTF、HTML 和图片卡片右键菜单中的“智能操作…”保持禁用；开启后单选支持类型可打开独立可激活窗口，多选仍禁用。文本直接展示记录正文；图片先展示缓存缩略图，不自动分析，只有用户选择具体操作后才读取缓存原图。

图片操作按能力分流：提取文字与翻译图片文字按需执行 Vision `VNRecognizeTextRequest`，二维码与条码识别执行 `VNDetectBarcodesRequest`；macOS 27 及以上且设备端模型声明支持视觉输入时，还可把原图作为 Foundation Models `Attachment` 生成客观图片描述。OCR 无结果时可以使用采集时随图片保留的文字附注。总入口要求苹果智能开关已开启；进入窗口后，翻译和图片描述继续检查对应设备端模型能力。

`LocalSmartActionGenerator` 按用户选择使用独立的结构化生成 schema：总结、改写、翻译和自定义指令只产生文本；日历事件产生标题、起止时间、地点和备注；邮件产生收件人、主题和正文。正文是不可信数据，不能选择动作或触发执行。相对日期使用记录采集时间和当前时区解析，日期与时间证据必须能在原文中找到。

文本的五个操作和图片最多四个操作固定在窗口底部，以单行等宽横向托盘排列；各操作图标使用共享的品牌暖棕加深色背景与白色图形。原文、原图、处理状态、草稿和结果位于托盘上方。文本原文使用固定高度的可滚动视口，处理阶段切换不会改变原文区高度；主操作按钮沿用相同的高对比配色。所有草稿和图片分析结果都先进入可编辑确认状态。OCR 与条码结果完整保存在结果字段中，不做行数截断；内容超过窗口时由隐藏滚动指示器的面板继续承载。文本结果仅在用户点击后复制；日历事件仅在用户点击确认后通过 EventKit 请求仅写权限并保存到默认日历；邮件只交给系统分享服务打开草稿，不发送。异步图片分析或生成在切换操作或窗口关闭时取消，防止迟到结果覆盖当前状态。
