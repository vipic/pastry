...
文本、RTF、HTML 和图片卡片的右键菜单只暴露统一的“智能操作…”入口，多选时禁用。入口先关闭托盘并打开可激活的独立窗口。文本直接展示记录正文；图片先展示缓存缩略图，不自动分析，只有用户选择具体操作后才读取缓存原图。

图片操作按能力分流：提取文字与翻译图片文字按需执行 Vision `VNRecognizeTextRequest`，二维码与条码识别执行 `VNDetectBarcodesRequest`；macOS 27 及以上且设备端模型声明支持视觉输入时，还可把原图作为 Foundation Models `Attachment` 生成客观图片描述。OCR 无结果时可以使用采集时随图片保留的文字附注。提取、条码及复制不要求 Apple Intelligence 可用，翻译和图片描述检查对应设备端模型能力。
...
所有草稿和图片分析结果都先进入可编辑确认状态。OCR 与条码结果完整保存在结果字段中，不做行数截断；内容超过窗口时由隐藏滚动指示器的面板继续承载。文本结果仅在用户点击后复制；日历事件仅在用户点击确认后通过 EventKit 请求仅写权限并保存到默认日历；邮件只交给系统分享服务打开草稿，不发送。异步图片分析或生成在切换卡片、返回动作选择或窗口关闭时取消，防止迟到结果覆盖当前状态。
...
- 底部托盘在宽屏使用横向卡带；左右托盘固定使用纵向列表并占用屏幕可见高度。每次显示托盘都按设置中的固定位置重新布局，并重置到当前结果列表起点：底部卡带回到最左侧，左右托盘回到最顶部，避免复用面板时继承上次滚动位置。实验功能中的紧凑列表开关缺省关闭；开启后左右托盘内容宽度从 320 pt 收窄到 272 pt，面板连同两侧 inset 共 296 pt，并把 240 pt 方形卡片替换为 84 pt 紧凑条目。侧边搜索展开宽度必须从固定托盘宽度扣除卡片容器内边距、头部内边距和右侧工具按钮，不能反向撑宽托盘。纯文本和富文本保持全宽文字布局；链接显示抓取的网页标题和链接地址；带图片的 HTML、图片记录与单文件在右侧使用 92 pt 宽的预览，左侧沿用类型、名称、时间和来源应用布局；多文件不显示右侧大预览，改为与完整卡片一致的文件图标和名称纵向列表。侧边头部把搜索与设置按钮相对托盘外框保持 14 pt inset，使 10 pt 控件圆角与 24 pt 托盘圆角共享圆心。紧凑模式所有条目的悬停轻操作始终锚定右下角并共用同一组内边距和 Liquid Glass 底板；底板由系统根据下方内容自动调整明暗，左右分栏条目可直接覆盖在预览区域上，不得通过预览宽度把按钮推回左侧信息区。常规卡片保留无统一底板的既有样式。两种侧边视图共用选择、拖拽、收藏、复制、删除、右键菜单和键盘导航，底部卡带不受该开关影响。拖拽停靠只在当前显示会话内生效；关闭后再次打开时始终恢复设置面板选择的位置。
