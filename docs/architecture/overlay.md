...
Esc 按当前 UI 层级逐层处理：确认层、备注编辑、筛选、Quick Look、搜索，最后才关闭面板。关闭搜索只清空搜索关键字并退出输入态，保留来源、类型、时间、备注等结构化筛选；这些筛选只由筛选气泡中的明确清除操作或面板整体重置移除。同一次 Esc 关闭 Quick Look 或备注编辑后可能级联触发第二次 cancel，管理器用短暂的吞键窗口阻止面板被一并关闭。
...
- 底部托盘在宽屏使用横向卡带；左右托盘固定使用纵向列表并占用屏幕可见高度。每次显示托盘都按设置中的固定位置重新布局，并重置到当前结果列表起点：底部卡带回到最左侧，左右托盘回到最顶部，避免复用面板时继承上次滚动位置。实验功能中的紧凑列表开关缺省关闭；开启后左右托盘内容宽度从 320 pt 收窄到 272 pt，面板连同两侧 inset 共 296 pt，并把 240 pt 方形卡片替换为 84 pt 紧凑条目。纯文本和富文本保持全宽文字布局；链接显示抓取的网页标题和链接地址；带图片的 HTML、图片记录与单文件在右侧使用 92 pt 宽的预览，左侧沿用类型、名称、时间和来源应用布局；多文件不显示右侧大预览，改为与完整卡片一致的文件图标和名称纵向列表。侧边头部把搜索与设置按钮相对托盘外框保持 14 pt inset，使 10 pt 控件圆角与 24 pt 托盘圆角共享圆心。紧凑模式所有条目的悬停轻操作始终锚定右下角并共用同一组内边距和 Liquid Glass 底板；底板由系统根据下方内容自动调整明暗，左右分栏条目可直接覆盖在预览区域上，不得通过预览宽度把按钮推回左侧信息区。常规卡片保留无统一底板的既有样式。两种侧边视图共用选择、拖拽、收藏、复制、删除、右键菜单和键盘导航，底部卡带不受该开关影响。拖拽停靠只在当前显示会话内生效；关闭后再次打开时始终恢复设置面板选择的位置。
