...
- General 的托盘打开位置直接保存并显示“位于屏幕左侧”“位于屏幕右侧”和“位于屏幕底侧”；拖动吸附只改变当前显示会话，关闭后再次打开时恢复设置位置。卡片点击模式的当前选项使用共享的品牌暖棕加深色与白字选中态。旧版“固定在底部”迁移为底侧，旧版“跟随记忆”迁移为当时保存的位置，迁移后移除旧模式键。左右侧边托盘固定使用紧凑列表，不再保存或展示布局开关。
