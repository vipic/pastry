...
- **苹果智能**：在设置中开启后，可使用智能找回和每张卡片右键菜单中的智能操作。智能找回分别理解搜索主题和明确条件，结合本地关键词、语义召回与候选重排定位历史，并可查看本次解析结果；智能操作支持文本生成，以及图片文字提取、二维码与条码识别和设备支持时的图片描述。
- **场景备注**：任何条目都可以添加备注，临时记录当时复制这段内容的场景。
- **链接预览**：复制链接后可抓取标题、描述和缩略图，方便回看。
- **文件预览**：文件卡片支持预览和定位原始文件位置。
- **多选操作**：支持区间选择、散选、批量删除和拖拽。
- **隐私排除**：支持按识别到的来源应用排除，并跳过带敏感、临时或自动生成标记的内容；没有来源标记的后台写入仍可能误归到前台应用，详见[来源识别边界](docs/architecture/clipboard-history.md#来源识别的能力边界)。
- **本地存储**：数据保存在本机，使用 SQLite + FTS5 管理与搜索历史。
