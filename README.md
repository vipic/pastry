...
- **智能找回（实验性）**：在通用设置中主动开启后，可在搜索框中描述来源、时间、类型或概念线索，由 Mac 设备端模型结合本地语义标签、句向量和候选重排找回相关历史；默认关闭。
- **智能操作**：对单条文本或图片右键选择“智能操作…”；图片保持原图预览，可按需提取或翻译文字、识别二维码与条码，并在支持视觉能力的系统上生成图片描述，所有结果均可完整检查、编辑和复制。
